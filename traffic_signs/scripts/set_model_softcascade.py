...
"""
Given a model and a cascade_threshold.txt file created from test_objects_detection,
...
def parse_arguments():
    ...
    parser.add_option("-o", "--output", dest="output_path",
                       ...
                       help="path to model to be created")
...
def create_updated_model(input_path, cascades_thresholds_path, output_path):
    ...
    if len(thresholds.shape) != 1:
        raise Exception("The number of cascade threshold lines is bigger than 1, check you cascade file")
...
def main():
    ...
    create_updated_model(options.input_path, options.cascades_thresholds_path, options.output_path)

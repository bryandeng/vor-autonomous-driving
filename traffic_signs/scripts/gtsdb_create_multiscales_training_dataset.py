...
"""
This program parses the GTSDB dataset and creates
...
def annotations_to_filenames_and_boxes(data_path, target_category):
    """
    We expect the data_path to be ....../TrainIJCNN2013.
    will return file path and box coordinates for a given
    category
    """

    annotations_file_path = os.path.join(data_path, "gt.txt")

    cat_pairs = [('Prohibitive', 0), ('Prohibitive', 1), ('Prohibitive', 2), \
                 ('Prohibitive', 3), ('Prohibitive', 4), ('Prohibitive', 5), \
                 ('Prohibitive', 7), ('Prohibitive', 8), ('Prohibitive', 9), \
                 ('Prohibitive', 10), ('Prohibitive', 15), ('Prohibitive', 16), \
                 ('Mandatory', 33), ('Mandatory', 34), ('Mandatory', 35), \
                 ('Mandatory', 36), ('Mandatory', 37), ('Mandatory', 38), \
                 ('Mandatory', 39), ('Mandatory', 40), \
                 ('Danger', 11), ('Danger', 18), ('Danger', 19), \
                 ('Danger', 20), ('Danger', 21), ('Danger', 22), \
                 ('Danger', 23), ('Danger', 24), ('Danger', 25), \
                 ('Danger', 26), ('Danger', 27), ('Danger', 28), \
                 ('Danger', 29), ('Danger', 30), ('Danger', 31) ]

    categories = defaultdict(list)
    for cat, cat_no in cat_pairs:
        categories[cat].append(cat_no)
    ...
    for line in f:
        ...
        seg = line.split(";")
        # see if the sign is in the category we want
        if int(seg[5].strip()) in categories[target_category]:
            image_path = os.path.join(data_path, seg[0])
            box = Box()
            box.min_corner.x = int(seg[1])
            box.min_corner.y = int(seg[2])
            box.max_corner.x = int(seg[3])
            box.max_corner.y = int(seg[4])
            boxes.append(box)
            annotations.append((image_path, boxes))
    #    if not boxes:
...
def parse_arguments():
    ...
    parser.add_option("-c", "--category", dest="traffic_sign_category",
                       metavar="CATEGORY", type="string",
                       help="category of traffic signs")
    ...
    if options.traffic_sign_category:
        if options.traffic_sign_category not in ['Prohibitive', 'Mandatory', 'Danger']:
            parser.error("Unknown traffic sign category")
    else:
        parser.error("you must pick one category of traffic signs")
...
def create_positives_process_annotation_box(box_counter, annotation_box, positives_path,
    ...
    object_height = 48 # the expected height of the traffic sign inside the detection box
...
def create_positives(model_width, model_height,
    ...
    # (filename, box)
    # each box represent a traffic sign
...
def create_negatives_process_file(negatives_counter, file_path, model_width, model_height, negatives_path):
    ...
    if (negative_image.shape[1] <= minimum_image_shape[1]) or  \
        ...
        if (negative_image.shape[1] <= minimum_image_shape[1]):
            #TODO: modify the way of resizing
            resized_size = (int(minimum_image_shape[1]),
                            int(minimum_image_shape[1]/image_ratio))
            #resized_size = (int(minimum_image_shape[1]*image_ratio),
                            #int(minimum_image_shape[1]))
...
def main():
    ...
    if False:
        ...
        create_multiscales_training_dataset(options.input_path, options.output_path, options.traffic_sign_category)
